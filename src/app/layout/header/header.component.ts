...
export class HeaderComponent {
  ...
  constructor(private elementRef: ElementRef, private _method: MethodService, private route: Router, private _frontend: FrontendService, private _scripts: ScriptsMethodService, private _wishList: WishlistService){
...
hotelList:any;
  ...
  onSearch(){
    ...
    if(this.bookDate && this.place){
      ...
      if(this.cityId){
        this.route.navigate(['/hotel-list'], { queryParams });
        localStorage.setItem('search', JSON.stringify({city: this.cityId, cityName: this.cityName, checkin:this.checkIn, checkout:this.checkOut, adults:this.adults, child:this.children, num_rooms:this.rooms}))
      }
      this.onShowSearch();
    }else{
